 ...
 * all point at the same `curevanails` database). Used for operator-tunable
 * values that shouldn't require a redeploy — e.g. `recruit_notify_to`, the
 * address(es) that receive a "new application" alert, edited from the notify
 * dashboard (notifications-service) and read here by /api/recruit.
 *
 * Lazy-created, like the other tables in this project — no migration step.
 */
