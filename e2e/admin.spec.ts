...
test.describe("auth gate", () => {
	...
	for (const path of ["/admin", "/admin/talent", "/admin/waitlist", "/admin/email"]) {
...
test.describe("email dashboard (folded in from notify)", () => {
	test("signed-in admin reaches the email dashboard at /admin/email", async ({ page }) => {
		await login(page);
	...
	test("dashboard nav links are prefixed for the /admin/email mount", async ({ page }) => {
		await login(page);
		await page.goto("/admin/email");
		// Base-aware links (src/utils/email-nav.ts) must point under /admin/email,
		// not at the notify Worker's root, so navigation stays in the admin session.
		await expect(page.locator('a[href="/admin/email/settings"]').first()).toBeVisible();
		await expect(page.locator('a[href="/admin/email/recruit-alerts"]').first()).toBeVisible();
	});
